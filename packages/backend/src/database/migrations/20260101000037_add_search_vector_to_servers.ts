...
const INDEX_NAME = 'servers_search_vector_index';
  ...
  await knex.schema.alterTable(TABLE_NAME, (table) => {
    table.specificType(
      ...
      `tsvector GENERATED ALWAYS AS (
        setweight(to_tsvector('portuguese', coalesce(hostname, '')), 'A') ||
        setweight(to_tsvector('portuguese', coalesce(display_name, '')), 'B') ||
        setweight(to_tsvector('portuguese', coalesce(server_type, '')), 'C') ||
