...
const VALID_ENGINES: readonly string[] = ['postgresql', 'mysql', 'mariadb', 'sql_server', 'oracle', 'mongodb', 'redis', 'elasticsearch'];
const VALID_STATUSES: readonly string[] = ['active', 'maintenance', 'provisioning', 'deprecated', 'deactivated'];
const VALID_CRITICALITIES: readonly string[] = ['low', 'medium', 'high', 'critical'];
const ENVIRONMENT_SLUG_RE = /^[a-z0-9_-]+$/;
const DB_NAME_RE = /^[a-z0-9_-]+$/i;

const CSV_TEMPLATE = [
  'name,displayName,engine,version,environment,criticality,status,description',
  'db-prod-01,Banco Principal,postgresql,15.4,production,high,active,Banco de dados principal de producao',
  'db-staging,Staging,mysql,8.0,staging,medium,active,Banco de staging para testes',
  'cache-prod,Cache Redis,redis,,production,medium,active,Cache em memoria de producao',
].join('\n');
...
function parseAndValidate(csvText: string): ParsedRow[] {
  ...
  return lines.slice(1).map((line, idx) => {
    ...
    const engine = col(cells, 'engine');
    const environment = col(cells, 'environment');

    if (!name) {
      errors.push('name obrigatorio');
    } else if (!DB_NAME_RE.test(name)) {
      errors.push('name invalido (use letras, numeros, hifen ou underscore)');
    }

    if (!engine) {
      errors.push('engine obrigatorio');
    } else if (!VALID_ENGINES.includes(engine.toLowerCase())) {
      errors.push(`engine "${engine}" invalido — use: ${VALID_ENGINES.join(', ')}`);
    }
    ...
    if (errors.length > 0) {
      return { lineNumber, name, engine, environment, errors };
    }
    ...
    const data: CreateDatabaseInput = {
      ...
      environment,
      displayName: col(cells, 'displayname') || undefined,
      description: col(cells, 'description') || undefined,
      version: col(cells, 'version') || undefined,
    ...
    return { lineNumber, name, engine, environment, errors: [], data };
  });
}
...
export function DatabaseImportDialog({ isOpen, onClose }: DatabaseImportDialogProps) {
  ...
  async function handleImport(): Promise<void> {
    ...
    for (const row of validRows) {
      ...
      try {
        await createDatabase.mutateAsync(row.data);
  ...
  function downloadTemplate(): void {
    ...
    a.href = url;
    a.download = 'modelo-importacao-bancos.csv';
  ...
  const modalTitle =
    step === 'input' ? 'Importar Bancos de Dados em Massa'
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={6}
              placeholder={'name,engine,environment,...\ndb-prod-01,postgresql,production,...'}
          ...
          <div className="rounded border border-line bg-surface-raised/40 px-3 py-3 text-xs text-slate-400">
            ...
            <p className="font-mono">name, engine, environment</p>
            <div className="mt-2 space-y-0.5">
              <p><span className="text-slate-300">engine:</span> postgresql | mysql | mariadb | sql_server | oracle | mongodb | redis | elasticsearch</p>
            ...
            <p className="font-mono text-slate-500">displayName, version, criticality (low|medium|high|critical), status (active|maintenance|provisioning|deprecated|deactivated), description</p>
          </div>
                ...
                <table className="w-full text-xs">
                  <thead className="sticky top-0 bg-surface">
                    <tr className="border-b border-line">
                      ...
                      <th className="px-3 py-2 text-left font-medium text-slate-400">Engine</th>
                  ...
                  <tbody>
                    {parsedRows.map((row) => (
                      <tr key={row.lineNumber} className="border-b border-line/50 last:border-0">
                        ...
                        <td className="px-3 py-1.5 font-mono text-slate-200">{row.name || '—'}</td>
                        <td className="px-3 py-1.5 text-slate-300">{row.engine || '—'}</td>
              ...
              <tbody>
                {results.map((r) => (
                  <tr key={r.lineNumber} className="border-b border-line/50 last:border-0">
                    ...
                    <td className="px-3 py-1.5 font-mono text-slate-200">{r.name || '—'}</td>
                    <td className="px-3 py-1.5">
                      {r.status === 'success' ? (
